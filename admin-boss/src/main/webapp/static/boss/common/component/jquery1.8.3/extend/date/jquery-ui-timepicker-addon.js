...
function Timepicker() {
	$.datepicker.regional[""] = $.datepicker.regional["zh-CN"];
...
$.datepicker._updateDatepicker = function(inst) {
	if (typeof(inst.stay_open) !== 'boolean' || inst.stay_open === false) {
		...
		if(tp_inst){
			...
			/* TODO: 添加清空按钮, 暂时只支持单日期 */
			var showButtonPanel = this._get(inst, 'showButtonPanel');
			if(showButtonPanel) {
				var $buttonDiv = inst.dpDiv.find("div.ui-datepicker-buttonpane");
				var $btn = $buttonDiv.find("button:first").html('&#28165;&#31354;');
				var input = inst.input;
				$btn.data("_input", input);
				$btn[0].onclick = function(input){};
				$btn.click(function(){
					var $input = $btn.data("_input");
