...
function deleteTemplate(id) {
  MessageBoxExt.confirm("是否确定删除此模板?", function(){
    MessageBoxExt.ajax({
      type : "post",
      url : GV.ctxPath + "/template/deleteLogTemplate.action",
      data : {templateId : id},
      style : "redirect"
    });
  });
  ...
  <div class="content_main fontText">
    <div class="result">
      <div id="template_guide" class="tag">
          <a href="${ctxPath}/template/logTemplateMgr" class="on"><em>【日志模板管理】</em></a><span>|</span>
          <a href="${ctxPath}/template/workItemTemplateMgr "><em>【双权限审核模板管理】</em></a><span>|</span>
      </div>
      </div>
    <div class="search">
      <form action="${ctxPath}/template/logTemplateMgr.action" id="form" method="get">
        <div class="search_tit">
          <h2 class="fw fleft f14">查询操作日志模板</h2>
        </div>
    ...
    <div class="submenu">
      <dl style="float:right;padding-right: 30px; height: 0px;">
        <dd><a href="${ctxPath}/template/showAddTemplate.action?templateTag=log">添加日志模板</a></dd>
      </dl>
    </div>
    <eq:listable queryService="queryService" queryKey="queryOperationLogTemplate" formId="form" pageSize="20" html="class='list'">
      ...
      <eq:col title="操作" escape="false" width="80px">
          <a href="${ctxPath}/template/showUpdateTemplate.action?templateId=${templateId!""}&templateTag=log">修改</a>
